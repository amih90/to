...
export class CommandsManager {
  ...
  private static registerTransformers(
    context: vscode.ExtensionContext,
    transformers: Transformer[]
  ) {
    transformers.forEach(transformer => {
      const disposable = vscode.commands.registerTextEditorCommand(
        `to.${transformer.name}`,
        function (textEditor: vscode.TextEditor, edit: vscode.TextEditorEdit) {
          if (!textEditor) {
            return;
          }
  ...
  private static registerUtilities(
    context: vscode.ExtensionContext,
    utilities: Utility[]
  ) {
